...
IS_SINGLE_FILE = True 
NODE_FILE_NAME = "EasyVideoOutputNode.py" 
...
def addComfyUINodesToMapping():
    if IS_SINGLE_FILE:
        # Registro para archivo único
        node_path = os.path.join(extension_folder, NODE_FILE_NAME)
        if os.path.isfile(node_path):
            log(f"  -> Registering nodes from file: {NODE_FILE_NAME}")
            spec = importlib.util.spec_from_file_location("EasyVideoOutputNode", node_path)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            classes_names = [attr for attr in dir(module) if "Node" in attr]
            for class_name in classes_names:
                if class_name not in NODE_CLASS_MAPPINGS:
                    NODE_CLASS_MAPPINGS[class_name] = getattr(module, class_name)
                    NODE_DISPLAY_NAME_MAPPINGS[class_name] = humanReadableTextReg.sub(" \\1\\2", class_name)
                    log(f"  -> Registered {class_name} as {NODE_DISPLAY_NAME_MAPPINGS[class_name]}")
    else:
        # Registro para carpetas, lógica existente
        for nodeElement in os.listdir(extension_folder):
            if not nodeElement.startswith('__') and nodeElement.endswith('Node') and os.path.isdir(os.path.join(extension_folder, nodeElement)):
                ...
                for f in os.listdir(node_folder):
                    ...
                    if os.path.isfile(os.path.join(node_folder, f)) and not f.startswith('__') and ext[1] == '.py':
                        spec = importlib.util.spec_from_file_location(f.replace(ext[1], ''), os.path.join(node_folder, f))
                        module = importlib.util.module_from_spec(spec)
                        spec.loader.exec_module(module)
                        classes_names = [attr for attr in dir(module) if "Node" in attr]
                        for class_name in classes_names:
