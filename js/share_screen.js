...
const runtime = {
  ...
  startClipArea: function (id, video) {
    ...
    this.ssa.update = function () {
      ...
      // draw selection area
      ctx.beginPath();
      ctx.strokeStyle = "rgba(0, 255, 234, 1.0)";
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      this.addWidget(
        ...
        function (value, widget, node) {
          ...
          // Agregar transición para la animación suave
          if (!parentEl.style.transition) {
            parentEl.style.transition = "opacity 0.5s ease";
          }

          if (parentEl.style.opacity === "1" || !parentEl.style.opacity) {
            // Si está visible, ocultar
            parentEl.style.opacity = "0";
            parentEl.style.pointerEvents = "none";
            this.label = "Show Preview"; // Cambiar texto del botón
          } else {
            // Si está oculto, mostrar
            parentEl.style.opacity = "1";
            parentEl.style.pointerEvents = "auto";
            this.label = "Hide Preview"; // Cambiar texto del botón
          }
        },
        {
          value: "",
